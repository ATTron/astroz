...
// J2 RAAN drift reference (deg/day) from poliastro
const j2RaanReference = [_]struct {
    a: f64, // semi-major axis km
    e: f64, // eccentricity
    i: f64, // inclination deg
    drift: f64, // RAAN drift deg/day
}{
    .{ .a = 6778.0, .e = 0.00, .i = 51.6, .drift = -5.0026915162 }, // ISS
    .{ .a = 26560.0, .e = 0.00, .i = 55.0, .drift = -0.0387844213 }, // GPS
    .{ .a = 42164.0, .e = 0.00, .i = 0.0, .drift = -0.0134142979 }, // GEO
    .{ .a = 26600.0, .e = 0.74, .i = 63.4, .drift = -0.1471559241 }, // Molniya
    .{ .a = 7178.0, .e = 0.00, .i = 98.7, .drift = 0.9967325105 }, // Sun-sync
};

test "validate J2 RAAN drift rate against Vallado formula" {
    for (j2RaanReference) |ref| {
        const a = ref.a;
        const e = ref.e;
        const iRad = ref.i * constants.deg2rad;

        const n = @sqrt(muEarth / (a * a * a)); // mean motion rad/s
        const factor = (1.0 - e * e);
        const dOmegaDt = -1.5 * j2Earth * (rEarth / a) * (rEarth / a) * n * @cos(iRad) / (factor * factor);
        const dOmegaDegDay = dOmegaDt * constants.rad2deg * constants.secondsPerDay;

        // Allow 1% tolerance for numerical differences
        const relErr = @abs(dOmegaDegDay - ref.drift) / @max(@abs(ref.drift), 1e-10);
        try testing.expect(relErr < 0.01);
    }
}

// Orbital period reference (seconds) from poliastro
const periodReference = [_]struct {
    a: f64,
    period: f64,
}{
    .{ .a = 6578.137, .period = 5309.6433652980 }, // 200 km altitude
    .{ .a = 6778.137, .period = 5553.6242712522 }, // 400 km (ISS)
    .{ .a = 7178.137, .period = 6052.4135494921 }, // 800 km
    .{ .a = 26578.137, .period = 43121.8896690152 }, // GPS
    .{ .a = 42164.137, .period = 86163.9904971709 }, // GEO
};

test "validate orbital period against analytical formula" {
    for (periodReference) |ref| {
        const calculated = calculations.orbitalPeriod(muEarth, ref.a);
        const relErr = @abs(calculated - ref.period) / ref.period;
        try testing.expect(relErr < 1e-10);
    }
}

// Hohmann transfer reference (km/s) from poliastro
const hohmannReference = [_]struct {
    r1: f64,
    r2: f64,
    dv1: f64,
    dv2: f64,
}{
    .{ .r1 = 6778.137, .r2 = 42164.000, .dv1 = 2.3974702520, .dv2 = 1.4564868010 }, // LEO to GEO
    .{ .r1 = 6778.137, .r2 = 26560.000, .dv1 = 2.0113689169, .dv2 = 1.4036310002 }, // LEO to GPS
    .{ .r1 = 6578.137, .r2 = 6778.137, .dv1 = 0.0580651240, .dv2 = 0.0576319627 }, // LEO to higher LEO
};

test "validate Hohmann transfer delta-V" {
    for (hohmannReference) |ref| {
        const transfer = calculations.hohmannTransfer(muEarth, ref.r1, ref.r2);

        // Allow 0.1% tolerance
        try testing.expect(@abs(transfer.deltaV1 - ref.dv1) / ref.dv1 < 0.001);
        try testing.expect(@abs(transfer.deltaV2 - ref.dv2) / ref.dv2 < 0.001);
    }
}
